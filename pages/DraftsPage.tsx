...
import { PlusCircleIcon, TrashIcon, SaveIcon, DraftIcon, PlusIcon, MinusIcon, ClockIcon, PaperclipIcon, XIcon, ImageUpIcon, GripVerticalIcon } from '../constants';
...
const DraftsPage: React.FC = () => {
  ...
  const isDirty = () => {
    ...
    return (
        selectedDraft.title !== draftTitle ||
        selectedDraft.content !== draftContent ||
        JSON.stringify(selectedDraft.scriptLines) !== JSON.stringify(scriptLines) ||
        JSON.stringify(selectedDraft.attachments) !== JSON.stringify(attachments)
    );
  };
  ...
  const renumberScenes = (lines: ScriptLine[]): ScriptLine[] => {
    return lines.map((line, index) => ({
      ...line,
      scene: (index + 1).toString(),
    }));
  };
  ...
  const addLine = (index: number) => {
    const newLines = [...scriptLines];
    newLines.splice(index + 1, 0, { id: uuidv4(), scene: '', description: '', duration: 0 });
    setScriptLines(renumberScenes(newLines));
  };
      ...
      <div className="w-1/3 min-w-[280px] max-w-[350px] bg-card-bg shadow-lg rounded-xl p-4 flex flex-col border border-border-color">
        <div className="flex justify-between items-center mb-4">
          ...
          <button
            ...
            className="p-2 text-accent hover:bg-hover-bg rounded-full"
          ...
          {draftNotes.map(draft => (
            <div
              ...
              className={`p-3 rounded-lg cursor-pointer transition-colors
                          ${selectedDraftId === draft.id ? 'bg-accent text-white shadow-md' : 'bg-subtle-bg hover:bg-hover-bg'}`}
            >
      ...
      <div className="flex-grow bg-card-bg shadow-lg rounded-xl flex flex-col border border-border-color">
        {selectedDraft ? (
          <>
            ...
            <div className="overflow-y-auto flex-grow p-4">
                {selectedDraft.type === 'SCRIPT' ? (
                     <table className="w-full border-collapse">
                        <thead className="sticky top-0 bg-subtle-bg z-10">
                            <tr>
                                <th className="p-2 border-b border-border-color w-10"></th>
                        ...
                        <tbody>
                            {scriptLines.map((line, index) => (
                                <tr 
                                  ...
                                  className={`group hover:bg-hover-bg ${draggedIndex === index ? 'opacity-50' : ''}`}
                                >
                                    <td className="p-1 border-b border-border-color text-center text-slate-400 cursor-grab active:cursor-grabbing">
                                        <GripVerticalIcon size={18}/>
                                    </td>
                                    <td className="p-1 border-b border-border-color"><input type="text" value={line.scene} onChange={(e) => updateLine(index, 'scene', e.target.value)} className="w-full bg-transparent p-1 rounded focus:bg-card-bg focus:ring-1 focus:ring-accent outline-none text-text-primary"/></td>
                                    <td className="p-1 border-b border-border-color"><textarea value={line.description} onChange={(e) => updateLine(index, 'description', e.target.value)} className="w-full bg-transparent p-1 rounded focus:bg-card-bg focus:ring-1 focus:ring-accent outline-none resize-y min-h-[40px] text-sm text-text-primary" rows={1}/></td>
                                    <td className="p-1 border-b border-border-color"><input type="number" value={line.duration} onChange={(e) => updateLine(index, 'duration', e.target.value)} className="w-full bg-transparent p-1 rounded focus:bg-card-bg focus:ring-1 focus:ring-accent outline-none text-text-primary"/></td>
            ...
            <div className="p-4 border-t border-border-color">
                ...
                <div className="flex flex-wrap gap-4 items-center">
                    ...
                    <button
                        ...
                        className="w-24 h-24 border-2 border-dashed border-border-color rounded-md flex flex-col items-center justify-center text-text-secondary hover:bg-hover-bg hover:border-accent hover:text-accent transition-colors"
            ...
            <div className="p-4 border-t border-border-color bg-subtle-bg flex justify-between items-center">
