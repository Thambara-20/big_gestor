  ...
  const StatCard: React.FC<{ title: string; value: string | number; icon: React.ReactNode; isCurrency?: boolean; color?: string }> = 
    ...
    <div className="bg-card-bg p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 border border-border-color">
      <div className="flex items-center justify-between">
        ...
        <div className={`p-3 rounded-full bg-highlight-bg ${color}`}>
          {icon}
        </div>
      </div>
    </div>
        ...
        <div className="bg-card-bg p-6 rounded-xl shadow-lg border border-border-color">
          ...
          {jobStatusData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                {/* FIX: Recharts Pie chart label prop was causing a TypeScript error. The argument type is now explicitly set to 'any' to resolve the type inference issue, as 'percent' is a valid runtime property provided by recharts. */}
                <Pie isAnimationActive={true} animationDuration={500} data={jobStatusData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} labelLine={false} label={({ name, percent }: any) => `${name} (${(percent * 100).toFixed(0)}%)`}>
          ...
          {upcomingDeadlines.length > 0 ? (
            <ul className="space-y-3">
              {upcomingDeadlines.map(job => (
                <li key={job.id} className="p-3 bg-subtle-bg rounded-lg hover:bg-hover-bg transition-colors">
